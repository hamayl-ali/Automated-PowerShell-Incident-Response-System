...
Pure Python 3 stdlib for the built-in rules. Sigma/YARA are optional add-on
detection sources -- see sigma_engine.py and yara_engine.py.
"""
...
def analyze_event(event: dict, sigma_rules=None, yara_rules=None) -> dict:
    """
    Runs all detection sources against a single log event:
      1. Built-in regex rules (this file)
      2. Sigma rules (sigma_engine.py), if sigma_rules is provided
      3. YARA rules (yara_engine.py), if yara_rules is provided (a compiled
         ruleset from yara_engine.compile_rules())

    Sigma/YARA scan the raw command text AND any decoded base64 payload
    combined -- an encoded command that only reveals its intent once
    decoded would otherwise be invisible to simple substring/keyword rules.

    Returns the event enriched with a 'findings' list and 'max_severity'.
    Each finding carries a 'source' field: "regex" | "sigma" | "yara".
    """
    ...
    combined_text = cmdline + ("\n" + decoded if decoded else "")

    if sigma_rules:
        from sigma_engine import evaluate_rules as sigma_evaluate
        sigma_event = dict(event)
        sigma_event["script_block"] = combined_text  # so |contains rules see decoded content too
        findings += sigma_evaluate(sigma_event, sigma_rules)

    if yara_rules:
        from yara_engine import scan_text as yara_scan_text
        findings += yara_scan_text(yara_rules, combined_text)
...
def analyze_batch(events: list, sigma_rules=None, yara_rules=None) -> list:
    return [analyze_event(e, sigma_rules=sigma_rules, yara_rules=yara_rules) for e in events]
